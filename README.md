...
| Task | What it does |
|---|---|
| **AutoIt: Run current file** | `AutoIt3.exe $ZED_FILE` — runs the script. |
| **AutoIt: Check current file (Au3Check)** | Lints the file with the official `Au3Check.exe` — preview of what the LSP shows as you edit. |
| **AutoIt: Compile current file to .exe (Aut2Exe)** | Direct compile to a standalone Windows binary with defaults. |
| **AutoIt: Compile with Options (AutoIt3Wrapper GUI)** | Launches the SciTE-bundled `AutoIt3Wrapper.au3` with `/ShowGui` so you pick icon / version info / console mode before compile. |
...
| **AutoIt: Kill running script** | SciTE-equivalent of Ctrl+Break — terminates the `AutoIt3.exe` process running the current file. |
